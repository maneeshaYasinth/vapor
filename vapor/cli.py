...
def run(profile, forecast, days):
    ...
    try:
        with console.status("[dim]fetching cost data from AWS...[/dim]", spinner="dots"):
            groups, total = get_cost_and_usage(profile)
            fc = get_forecast(profile) if forecast else None
            daily = get_daily_costs(profile, days) if days else None
